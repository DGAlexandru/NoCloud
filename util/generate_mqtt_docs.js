...
const NoCloudEventStore = require("NoCloud-backend/lib/NoCloudEventStore");
...
const eventStore = new NoCloudEventStore()
...
class FakeMqttController extends MqttController {
    ...
    async generateHandleDoc(handle, markdownLevel, recurse) {
        ...
        if (handle.constructor.OPTIONAL === true) {
            markdown += `**Note:** This is an optional exposed capability handle and thus will only be available via MQTT if enabled in the NoCloud configuration.\n\n`;
        }
    ...
    async doGenerateDocs() {
        ...
        try {
            ...
            const NoCloudEvents = this.crawlGetHandlesOfType(this.robotHandle, NoCloudEventsNodeMqttHandle);
            ...
            const NoCloudEventsRes = await this.generateHandleDoc(NoCloudEvents[0], 3, true);
            markdown += NoCloudEventsRes.markdown;
            anchors.children.push(NoCloudEventsRes.anchors);
            Object.assign(hassComponentAnchors, NoCloudEventsRes.hassComponentAnchors);
            Object.assign(stateAttrAnchors, NoCloudEventsRes.stateAttrAnchors);
