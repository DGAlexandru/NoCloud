...
const NoCloudMapDataExportButtonItem = (): React.ReactElement => {
    const {
    ...
    return (
        <ButtonListMenuItem
            primaryLabel="Export NoCloudMap"
            secondaryLabel="Download a NoCloudMap data export to use with other tools"
            icon={<NoCloudMapDownloadIcon/>}
            buttonLabel="Go"
            action={() => {
                if (NoCloudInformation && mapData) {
                    ...
                    linkElement.download = `NoCloudMapExport-${NoCloudInformation.systemId}-${timestamp}.json`;

                    linkElement.click();
                }
            }}
            actionLoading={NoCloudInformationPending || mapPending}
        />
    );
};
