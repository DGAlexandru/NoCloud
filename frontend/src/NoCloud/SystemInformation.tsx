...
import {
    ...
    useNoCloudInformationQuery, CPUUsageType,
} from "../api";
...
const SystemInformation = (): React.ReactElement => {
    ...
    const NoCloudInformationView = React.useMemo(() => {
        if (NoCloudInformationViewLoading) {
            return (
                <Skeleton height={"4rem"}/>
            );
        }

        if (!version && !NoCloudInformation) {
            return <Typography color="error">No NoCloud information</Typography>;
        }
        ...
        const items = [
            ...
            {
                header: "Embedded",
                body: NoCloudInformation?.embedded ? "true" : "false"
            },
            {
                header: "System ID",
                body: NoCloudInformation?.systemId
            }
        ].filter(item => {
                    ...
                    <RatioBar
                        ...
                        partitions={
                            [
                                {
                                    label: "System",
                                    value: systemHostInfo.mem.total - systemHostInfo.mem.free - systemHostInfo.mem.NoCloud_current,
                                    valueLabel: `${((systemHostInfo.mem.total - systemHostInfo.mem.free - systemHostInfo.mem.NoCloud_current) / 1024 / 1024).toFixed(2)} MiB`,
                                    color: "#7AC037"
                                },
                                {
                                    label: "NoCloud",
                                    value: systemHostInfo.mem.NoCloud_current,
                                    valueLabel: `${((systemHostInfo.mem.NoCloud_current) / 1024 / 1024).toFixed(2)} MiB`,
                                    color: "#DF5618"
                                },
                                {
                                    label: "NoCloud (Max)",
                                    value: systemHostInfo.mem.NoCloud_max - systemHostInfo.mem.NoCloud_current,
                                    valueLabel: `${((systemHostInfo.mem.NoCloud_max) / 1024 / 1024).toFixed(2)} MiB`,
                                    color: "#19A1A1"
                                }
                            ]
                        }
