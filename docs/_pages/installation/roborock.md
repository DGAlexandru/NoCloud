...
This works by using the official OTA update mechanism to push a customized (rooted + NoCloud) firmware image to the robot.
...
In DustBuilder, make sure to use the "Build update package" option, as it is the only file type supported by NoCloud-helper-miioota.

The service is provided by Dennis who is also the reason, why NoCloud can exist in the first place.
...
```
./NoCloud-helper-miioota install-firmware v11_2034.pkg 
Starting installer.
...
Push the downloaded NoCloud binary to the robot using `scp`. Note that there is no trailing slash in the sample command,
as `/mnt/data/NoCloud` must be the binary itself and **not** a folder containing the binary.
```
scp -O -i ~/.ssh/your_keyfile Downloads/NoCloud-armv7-lowmem.upx root@192.168.8.1:/mnt/data/NoCloud
```

Connect to the robot via `ssh`. You will now clean up the installer files and setup NoCloud to autostart on boot:
```
cd /mnt/data
rm roborock.vacuum.*.gz boot.img firmware.md5sum rootfs.img install.sh

cp /root/_root.sh.tpl /mnt/reserve/_root.sh
chmod +x /mnt/reserve/_root.sh /mnt/data/NoCloud
