...
In its most basic implementation, this capability allows enabling or disabling the automatic auto-emptying into the dock after a finished cleanup.

On more sophisticated firmwares, options such as only doing it once every few finished cleanups or even doing it multiple times per cleanup might be available.
