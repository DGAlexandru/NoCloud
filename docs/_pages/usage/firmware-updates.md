...
Historically speaking, at the time of writing (August 2026), new firmwares on supported bots only ever made those better, so you may want to actually update from time to time.
NoCloud also targets the latest firmware version available in the dustbuilder, so if something does not work, make sure you're on that.
