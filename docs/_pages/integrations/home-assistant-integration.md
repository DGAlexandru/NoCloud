...
Since NoCloud 2021.04.0 "vacuum.send_command" is no longer supported. It is replaced by the the `mqtt.publish` service.
...
These examples are written for a hypothetical NoCloud-enabled Robot with the System ID `JustForDemonstration`.<br/>
They need to be adapted to your System ID that can be found on the *System Information* page of your NoCloud instance.
...
service: vacuum.stop
target:
  entity_id: vacuum.NoCloud_justfordemonstration
```
...
For the segment cleaning capability, you should first go ahead to NoCloud and rename your segments. <br/>
Then you can go and check out the entity `sensor.NoCloud_justfordemonstration_map_segments`, which provides a list of your segments like this:
...
data:
  topic: NoCloud/JustForDemonstration/MapSegmentationCapability/clean/set
  ...
  - type: custom:button-card
    ...
    lock:
      enabled: >-
        ...
        states['vacuum.NoCloud_justfordemonstration'].state !== 'docked']]]
      exemptions: []
...
vacuum_trigger_autoempty:
  ...
  - service: mqtt.publish
    data:
      topic: NoCloud/JustForDemonstration/AutoEmptyDockManualTriggerCapability/trigger/set
      payload: PERFORM
  mode: single
{% endraw %}
```
...
If you are on Hass.io and want the map also on your dashboards of Home Assistant, you can use the [Lovelace NoCloud Map Card
](https://github.com/TheLastProject/lovelace-NoCloud-map-card).
