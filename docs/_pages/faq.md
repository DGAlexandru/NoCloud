...
In a similar fashion to the [Why not NoCloud?](./general/why-not-NoCloud.md) page,
...
NoCloud does not support bypassing region locks to e.g. use a CN unit outside of CN.
...
All the android one does is discover NoCloud instances and then open a browser.<br/>
...
The most important one being that Matter attempts to be a solution to a problem NoCloud simply doesn't have.
...
<br/>
<p>
The following was a reply by me to a reddit post titled "What do you folks think of NoCloud's criticism of Matter/Thread?":
</p>
