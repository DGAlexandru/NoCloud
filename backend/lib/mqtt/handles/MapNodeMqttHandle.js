            ...
            this.registerChild(
                new PropertyMqttHandle({
                    ...
                    helpText: "This handle is added automatically if Home Assistant autodiscovery is enabled. It " +
                        "provides a map embedded in a PNG image that recommends installing the NoCloud Lovelace card."
                }).also((prop) => {
                    prop.attachHomeAssistantComponent(
                        new InLineHassComponent({
                            hass: hass,
...
const PNG_WRAPPER = {
    ...
    TEXT_CHUNK_METADATA: Buffer.from("NoCloudMap\0\0"),
    IMAGE: fs.readFileSync(path.join(__dirname, "../../res/NoCloud_home_assistant_mqtt_wrapper.png"))
};
