            ...
            new PropertyMqttHandle({
                ...
                getter: async () => {
                    ...
                    if (statusState?.value === stateAttrs.StatusStateAttribute.VALUE.ERROR) {
                        ...
                        value = new NoCloudRobotError({
                            severity: {
                                kind: NoCloudRobotError.SEVERITY_KIND.NONE,
                                level: NoCloudRobotError.SEVERITY_LEVEL.NONE
                            },
                            subsystem: NoCloudRobotError.SUBSYSTEM.NONE,
                            message: "",
                            vendorErrorCode: "",
                        });
                    }
                ...
                helpText: "This property contains the current NoCloudRobotError (if any)"
            }).also((prop) => {
                this.controller.hassAnchorProvider.getTopicReference(
                    HassAnchor.REFERENCE.NoCloud_ROBOT_ERROR
                ).post(prop.getBaseTopic()).catch(err => {
                    Logger.error("Error while posting value to HassAnchor", err);
                });
            })
