...
class MqttController {
    ...
    constructor(options) {
        ...
        this.NoCloudEventStore.onEventsUpdated(() => {
            this.onNoCloudEventsUpdated();
        });
        ...
        this.NoCloudHelper.onFriendlyNameChanged(() => {
            this.handleConfigUpdated().catch((err) => {
    ...
    loadConfig() {
        ...
        this.currentConfig = structuredClone({
            clientId: "NoCloud_" + Tools.GET_HUMAN_READABLE_SYSTEM_ID(),
            ...
            this.client.on("connect", () => {
                ...
                this.reconfigure(async () => {
                    ...
                    this.setState(HomieCommonAttributes.STATE.READY).then(() => {
                        this.onNoCloudEventsUpdated(); // Publish the initial state
    ...
    onNoCloudEventsUpdated() {
        if (this.currentConfig.enabled && this.isInitialized && this.robotHandle !== null) {
            const NoCloudEventsHandle = this.robotHandle.getNoCloudEventsHandle();

            if (NoCloudEventsHandle !== null) {
                NoCloudEventsHandle.onNoCloudEventsUpdated();
            }
        }
    }
