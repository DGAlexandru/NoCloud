...
class NTPClient {
    ...
    constructor(options) {
        ...
        if (this.config.get("ntpClient").enabled) {
            this.state = new States.NoCloudNTPClientEnabledState({});
        } else {
            this.state = new States.NoCloudNTPClientDisabledState({});
        }

        // On startup, we need to wait for a while for NoCloud to fully start up (at least when using pkg) or else
    ...
    async pollTime() {
        ...
        try {
            ...
            this.state = new States.NoCloudNTPClientSyncedState({
            ...
            let error = {
                type: States.NoCloudNTPClientErrorState.ERROR_TYPE.UNKNOWN,
                message: e.message
            };
            ...
            if (typeof e.code === "string") {
                switch (e.code) {
                    ...
                    case "ECONNRESET":
                    case "EPIPE":
                        error.type = States.NoCloudNTPClientErrorState.ERROR_TYPE.TRANSIENT;
                        break;
                    case "ENOTFOUND":
                        error.type = States.NoCloudNTPClientErrorState.ERROR_TYPE.NAME_RESOLUTION;
                        break;
                }
            } else if (typeof e.message === "string" && e.message.indexOf("Timeout waiting") === 0) {
                error.type = States.NoCloudNTPClientErrorState.ERROR_TYPE.CONNECTION;
                error.message = e.message;
            } else if (e.stdout !== undefined) {
                error.type = States.NoCloudNTPClientErrorState.ERROR_TYPE.PERSISTING;
                error.message = e.stdout.toString() + " " + e.stderr.toString();
            }

            if (error.type !== States.NoCloudNTPClientErrorState.ERROR_TYPE.TRANSIENT) {
            ...
            this.state = new States.NoCloudNTPClientErrorState(error);
