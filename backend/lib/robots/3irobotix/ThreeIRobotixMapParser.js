...
class ThreeIRobotixMapParser {
    ...
    static PARSE_ROBOT_POSITION_BLOCK(block) {
        ...
        return {
            x: x,
            y: y,
            angle: ThreeIRobotixMapParser.CONVERT_TO_NoCloud_ANGLE(angle)
        };
    }
    ...
    static POST_PROCESS_BLOCKS(blocks, uniqueMapId) {
        if (blocks[TYPE_FLAGS.MAP_IMAGE]?.pixels) {
            ...
            if (blocks[TYPE_FLAGS.PATH]?.length > 0) {
                pathPoints = ThreeIRobotixMapParser.CONVERT_TO_NoCloud_COORDINATES(blocks[TYPE_FLAGS.PATH], mapWidth, mapHeight, pixelSize);
            ...
            if (blocks[TYPE_FLAGS.ROBOT_POSITION]) {
                ...
                entities.push(new mapEntities.PointMapEntity({
                    points: ThreeIRobotixMapParser.CONVERT_TO_NoCloud_COORDINATES([x, y], mapWidth, mapHeight, pixelSize),
                    metaData: {
            ...
            if (blocks[TYPE_FLAGS.VIRTUAL_RESTRICTIONS]) {
                blocks[TYPE_FLAGS.VIRTUAL_RESTRICTIONS].walls.forEach((wall) => {
                    entities.push(new mapEntities.LineMapEntity({
                        points: ThreeIRobotixMapParser.CONVERT_TO_NoCloud_COORDINATES(wall, mapWidth, mapHeight, pixelSize),
                ...
                blocks[TYPE_FLAGS.VIRTUAL_RESTRICTIONS].areas.forEach((area) => {
                    entities.push(new mapEntities.PolygonMapEntity({
                        points: ThreeIRobotixMapParser.CONVERT_TO_NoCloud_COORDINATES(area, mapWidth, mapHeight, pixelSize),
            ...
            if (blocks[TYPE_FLAGS.ACTIVE_ZONES]) {
                blocks[TYPE_FLAGS.ACTIVE_ZONES].areas.forEach((activeZone) => {
                    entities.push(new mapEntities.PolygonMapEntity({
                        points: ThreeIRobotixMapParser.CONVERT_TO_NoCloud_COORDINATES(activeZone, mapWidth, mapHeight, pixelSize),
     ...
     */
    static CONVERT_TO_NoCloud_COORDINATES(points, mapWidth, mapHeight, pixelSize) {
     ...
     * @return {number}
     */
    static CONVERT_TO_NoCloud_ANGLE(angle) {
