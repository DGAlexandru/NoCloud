...
class DreameMapParser {
     ...
     * @returns {Promise<null|import("../../entities/map/NoCloudMap")>}
     */
    static async PARSE(buf, mapType = MAP_DATA_TYPES.REGULAR) {
        ...
        if (parsedHeader.robot_position.valid === true) {
            entities.push(
                new mapEntities.PointMapEntity({
                    ...
                    metaData: {
                        angle: DreameMapParser.CONVERT_ANGLE_TO_NoCloud(parsedHeader.robot_position.angle)
                    },
        ...
        if (parsedHeader.charger_position.valid === true) {
            entities.push(
                new mapEntities.PointMapEntity({
                    ...
                    metaData: {
                        angle: DreameMapParser.CONVERT_ANGLE_TO_NoCloud(parsedHeader.charger_position.angle)
                    },
        ...
        if (buf.length >= HEADER_SIZE + parsedHeader.width * parsedHeader.height) {
            ...
            if (additionalData.rism && (additionalData.ris === 2 || additionalData.iscleanlog === true)) {
                ...
                if (rismResult instanceof mapEntities.NoCloudMap) {
            ...
            if (additionalData.pointinfo && Array.isArray(additionalData.pointinfo.tpoint) && additionalData.pointinfo.tpoint.length === 1) {
                const goToPoint = DreameMapParser.CONVERT_TO_NoCloud_COORDINATES(
            ...
            if (additionalData.ai_obstacle?.length > 0) {
                additionalData.ai_obstacle.forEach((obstacle) => {
                    const coords = DreameMapParser.CONVERT_TO_NoCloud_COORDINATES(
            ...
            if (additionalData.carpet_info) {
                for (const [carpetId, carpetInfo] of Object.entries(additionalData.carpet_info)) {
                    const pA = DreameMapParser.CONVERT_TO_NoCloud_COORDINATES(carpetInfo[0], carpetInfo[1]);
                    const pB = DreameMapParser.CONVERT_TO_NoCloud_COORDINATES(carpetInfo[2], carpetInfo[3]);
            ...
            if (additionalData.carpet_polygon) {
                for (const [carpetId, carpetPolygon] of Object.entries(additionalData.carpet_polygon)) {
                    ...
                    for (let i = 0; i < coords.length; i = i + 2) {
                        const p = DreameMapParser.CONVERT_TO_NoCloud_COORDINATES(coords[i], coords[i+1]);
        ...
        return new mapEntities.NoCloudMap({
            metaData: metaData,
            size: {
                x: MAX_X,
                y: MAX_Y
    ...
    static PARSE_HEADER(buf) {
        ...
        parsedHeader.robot_position = DreameMapParser.CONVERT_TO_NoCloud_COORDINATES(buf.readInt16LE(5), buf.readInt16LE(7));
        parsedHeader.robot_position.angle = buf.readInt16LE(9);
        parsedHeader.robot_position.valid = true;

        parsedHeader.charger_position = DreameMapParser.CONVERT_TO_NoCloud_COORDINATES(buf.readInt16LE(11), buf.readInt16LE(13));
    ...
    static PARSE_IMAGE(parsedHeader, activeSegmentIds, segmentNames, segmentMaterials, buf, mapType) {
        ...
        for (let i = 0; i < parsedHeader.height; i++) {
            for (let j = 0; j < parsedHeader.width; j++) {
                ...
                /**
                 * The NoCloud map origin is in the top left corner
    ...
    static PARSE_PATH(parsedHeader, traceString, appendRobotPosition) {
        ...
        unprocessedPaths.forEach((unprocessedPoints, i) => {
            ...
            unprocessedPoints.forEach(e => {
                const p = DreameMapParser.CONVERT_TO_NoCloud_COORDINATES(e.x, e.y);
    ...
    static PARSE_AREAS(parsedHeader, areas, type) {
        return areas.map(a => {
            const pA = DreameMapParser.CONVERT_TO_NoCloud_COORDINATES(a[0], a[1]);
            const pB = DreameMapParser.CONVERT_TO_NoCloud_COORDINATES(a[2], a[3]);
    ...
    static PARSE_RAMPS(parsedHeader, ramps) {
        return ramps.map(r => {
            const pA = DreameMapParser.CONVERT_TO_NoCloud_COORDINATES(r[0], r[1]);
            const pB = DreameMapParser.CONVERT_TO_NoCloud_COORDINATES(r[2], r[3]);
            const angle = r[4];
...
/**
 * Dreame coordinates are signed INT16. NoCloud coordinates are unsigned
 ...
 */
DreameMapParser.CONVERT_TO_NoCloud_COORDINATES = function(x, y) {
    return {
 ...
 * @return {number}
 */
DreameMapParser.CONVERT_ANGLE_TO_NoCloud = function(angle) {
