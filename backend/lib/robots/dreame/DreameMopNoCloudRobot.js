...
class DreameMopNoCloudRobot extends DreameGen2NoCloudRobot {
    constructor(options) {
        super(options);
        ...
        this.registerCapability(new capabilities.DreameMopDockWaterUsageControlCapability({
            ...
            presets: Object.keys(WATER_GRADES).map(k => {
                return new NoCloudSelectionPreset({name: k, value: WATER_GRADES[k]});
            }),
            siid: DreameGen2NoCloudRobot.MIOT_SERVICES.VACUUM_2.SIID,
    ...
    parseAndUpdateState(data) {
        ...
        data.forEach(elem => {
            switch (elem.siid) {
                case DreameGen2NoCloudRobot.MIOT_SERVICES.VACUUM_2.SIID: {
                    switch (elem.piid) {
                        case DreameGen2NoCloudRobot.MIOT_SERVICES.VACUUM_2.PROPERTIES.MOP_DOCK_SETTINGS.PIID: {
...
module.exports = DreameMopNoCloudRobot;
