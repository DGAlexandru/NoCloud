...
class MiioNoCloudRobot extends NoCloudRobot {
    /**
     *
    ...
    get deviceId() {
        ...
        if (!deviceId && this.config.get("embedded") === true) {
            deviceId = MiioNoCloudRobot.READ_DEVICE_CONF(this.deviceConfPath)["did"];
        }
    ...
    pollMap() {
        this.mapPollMutex.take(() => {
            let repollSeconds = MiioNoCloudRobot.MAP_POLLING_INTERVALS.DEFAULT;
            ...
            this.executeMapPoll().then((response) => {
                ...
                repollSeconds = MiioNoCloudRobot.MAP_POLLING_INTERVALS.ERROR;
            }).finally(() => {
     ...
     * @abstract
     * @param {Buffer} data
     * @returns {Promise<import("../entities/map/NoCloudMap")|null>}
     */
    async parseMap(data) {
        throw new NotImplementedError();
    }
    ...
    getCloudSecretFromFS() {
        return MiioNoCloudRobot.READ_DEVICE_CONF(this.deviceConfPath)["key"];
    }
...
MiioNoCloudRobot.MAP_POLLING_INTERVALS = Object.freeze({
    DEFAULT: 60,
    ACTIVE: 2,
    ERROR: 30
});

module.exports = MiioNoCloudRobot;
