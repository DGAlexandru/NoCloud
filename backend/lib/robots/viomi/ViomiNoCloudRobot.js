...
class ViomiNoCloudRobot extends MiioNoCloudRobot {
    /**
     *
    ...
    constructor(options) {
        ...
        this.registerCapability(new capabilities.ViomiOperationModeControlCapability({
            ...
            presets: Object.keys(attributes.ViomiOperationMode).map(k => {
                return new NoCloudSelectionPreset({name: k, value: attributes.ViomiOperationMode[k]});
            }),
        }));
    ...
    setEmbeddedParameters() {
        this.deviceConfPath = ViomiNoCloudRobot.DEVICE_CONF_PATH;
        this.tokenFilePath = ViomiNoCloudRobot.TOKEN_FILE_PATH;
    }
        ...
        if (
            ...
            if (error !== undefined) {
                if (error.value === stateAttrs.StatusStateAttribute.VALUE.ERROR) {
                    ...
                    statusError = new NoCloudRobotError({
                        severity: {
...
ViomiNoCloudRobot.DEVICE_CONF_PATH = "/etc/miio/device.conf";
ViomiNoCloudRobot.TOKEN_FILE_PATH = "/etc/miio/device.token";
...
module.exports = ViomiNoCloudRobot;
