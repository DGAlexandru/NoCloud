...
/**
 * @extends ZoneCleaningCapability<import("../RoborockNoCloudRobot")>
 */
class RoborockZoneCleaningCapability extends ZoneCleaningCapability {
    async start(options) {
        const flippedRoborockZones = options.zones.map(NoCloudZone => {
            const yFlippedZone = [
                Math.floor(NoCloudZone.points.pA.x * 10),
                Math.floor(RoborockMapParser.DIMENSION_MM - NoCloudZone.points.pA.y * 10),
                Math.floor(NoCloudZone.points.pC.x * 10),
                Math.floor(RoborockMapParser.DIMENSION_MM - NoCloudZone.points.pC.y * 10),
