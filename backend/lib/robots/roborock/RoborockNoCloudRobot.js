...
class RoborockNoCloudRobot extends MiioNoCloudRobot {
    /**
     *
    ...
    constructor(options) {
        ...
        this.registerCapability(new capabilities.RoborockFanSpeedControlCapability({
            ...
            presets: Object.keys(this.fanSpeeds).map(k => {
                return new NoCloudSelectionPreset({name: k, value: this.fanSpeeds[k]});
            })
        }));
    ...
    setEmbeddedParameters() {
        this.deviceConfPath = RoborockNoCloudRobot.DEVICE_CONF_PATH;
        this.tokenFilePath = RoborockNoCloudRobot.TOKEN_FILE_PATH;
    }
    ...
    onIncomingCloudMessage(msg) {
        switch (msg.method) {
            ...
            case "event.bin_full":
                this.NoCloudEventStore.raise(new DustBinFullNoCloudEvent({}));
            ...
            case "event.remind_to_save_map":
                this.NoCloudEventStore.raise(new PendingMapChangeNoCloudEvent({}));
    ...
    parseAndUpdateState(data) {
        ...
        if (data["state"] !== undefined && STATUS_MAP[data["state"]]) {
            ...
            if (
                ...
                statusError = RoborockNoCloudRobot.MAP_ERROR_CODE(data["error_code"]);
            }
    ...
    async parseMap(data) {
        ...
        if (parsedMap instanceof NoCloudMap) {
...
RoborockNoCloudRobot.DEVICE_CONF_PATH = "/mnt/default/device.conf";
RoborockNoCloudRobot.TOKEN_FILE_PATH = "/mnt/data/miio/device.token";
 ...
 * 
 * @returns {NoCloudRobotError}
 */
RoborockNoCloudRobot.MAP_ERROR_CODE = (vendorErrorCode) => {
    const parameters = {
        severity: {
            kind: NoCloudRobotError.SEVERITY_KIND.UNKNOWN,
            level: NoCloudRobotError.SEVERITY_LEVEL.UNKNOWN,
        },
        subsystem: NoCloudRobotError.SUBSYSTEM.UNKNOWN,
    ...
    switch (vendorErrorCode) {
        ...
        case 5:
            parameters.severity.kind = NoCloudRobotError.SEVERITY_KIND.PERMANENT;
            parameters.severity.level = NoCloudRobotError.SEVERITY_LEVEL.ERROR;
            parameters.subsystem = NoCloudRobotError.SUBSYSTEM.MOTORS;
        ...
        case 11:
            parameters.severity.kind = NoCloudRobotError.SEVERITY_KIND.TRANSIENT;
            parameters.severity.level = NoCloudRobotError.SEVERITY_LEVEL.INFO;
            parameters.subsystem = NoCloudRobotError.SUBSYSTEM.SENSORS;
        ...
        case 29:
            parameters.severity.kind = NoCloudRobotError.SEVERITY_KIND.TRANSIENT;
            parameters.severity.level = NoCloudRobotError.SEVERITY_LEVEL.CATASTROPHIC;
            parameters.subsystem = NoCloudRobotError.SUBSYSTEM.UNKNOWN;
    ...
    return new NoCloudRobotError(parameters);
};
...
module.exports = RoborockNoCloudRobot;
