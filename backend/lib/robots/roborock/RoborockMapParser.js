...
class RoborockMapParser {
     ...
     * @returns {null|import("../../entities/map/NoCloudMap")}
     */
    static PARSE(mapBuf){
    ...
    static POST_PROCESS_BLOCKS(metaData, blocks) {
        if (blocks[BlockTypes.IMAGE] && blocks[BlockTypes.IMAGE].pixels) { //We need the image to flip everything else correctly
            ...
            if (blocks[BlockTypes.PATH]) {
                const points = TransformRoborockCoordinateArraysToNoCloudCoordinateArrays(blocks[BlockTypes.PATH].points);
            ...
            if (blocks[BlockTypes.GOTO_PREDICTED_PATH]) {
                const predictedPathPoints = TransformRoborockCoordinateArraysToNoCloudCoordinateArrays(blocks[BlockTypes.GOTO_PREDICTED_PATH].points);
            ...
            if (blocks[BlockTypes.CHARGER_LOCATION]) {
                entities.push(new mapEntities.PointMapEntity({
                    points: TransformRoborockCoordinateArraysToNoCloudCoordinateArrays(blocks[BlockTypes.CHARGER_LOCATION].position),
            ...
            if (blocks[BlockTypes.ROBOT_POSITION]) {
                ...
                entities.push(new mapEntities.PointMapEntity({
                    points: TransformRoborockCoordinateArraysToNoCloudCoordinateArrays(blocks[BlockTypes.ROBOT_POSITION].position),
                    metaData: {
                        angle: angle
                    },
            ...
            if (blocks[BlockTypes.GOTO_TARGET]) {
                entities.push(new mapEntities.PointMapEntity({
                    points:TransformRoborockCoordinateArraysToNoCloudCoordinateArrays(blocks[BlockTypes.GOTO_TARGET].position),
            ...
            if (blocks[BlockTypes.CURRENTLY_CLEANED_ZONES]) {
                blocks[BlockTypes.CURRENTLY_CLEANED_ZONES].forEach(zone => {
                    zone = TransformRoborockCoordinateArraysToNoCloudCoordinateArrays(zone);
            ...
            if (blocks[BlockTypes.NO_GO_AREAS]) {
                blocks[BlockTypes.NO_GO_AREAS].forEach(area => {
                    entities.push(new mapEntities.PolygonMapEntity({
                        points: TransformRoborockCoordinateArraysToNoCloudCoordinateArrays(area),
            ...
            if (blocks[BlockTypes.VIRTUAL_WALLS]) {
                blocks[BlockTypes.VIRTUAL_WALLS].forEach(wall => {
                    entities.push(new mapEntities.LineMapEntity({
                        points: TransformRoborockCoordinateArraysToNoCloudCoordinateArrays(wall),
            ...
            if (blocks[BlockTypes.OBSTACLES]) {
                blocks[BlockTypes.OBSTACLES].forEach(obstacle => {
                    entities.push(new mapEntities.PointMapEntity({
                        points: TransformRoborockCoordinateArraysToNoCloudCoordinateArrays(obstacle.position),
...
function TransformRoborockCoordinateArraysToNoCloudCoordinateArrays(points) {
