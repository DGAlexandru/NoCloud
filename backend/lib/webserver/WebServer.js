...
class WebServer {
    ...
    constructor(options) {
        ...
        this.NoCloudRouter = new NoCloudRouter({config: this.config, robot: this.robot, validator: this.validator});

        this.app.use("/api/v2/robot/", this.robotRouter.getRouter());

        this.app.use("/api/v2/NoCloud/", this.NoCloudRouter.getRouter());
        ...
        this.app.use("/_ssdp/", new SSDPRouter({config: this.config, robot: this.robot, NoCloudHelper: this.NoCloudHelper}).getRouter());
    ...
    loadApiSpec() {
        ...
        try {
            spec = JSON.parse(fs.readFileSync(path.join(__dirname, "../res/NoCloud.openapi.schema.json")).toString());
        } catch (e) {
